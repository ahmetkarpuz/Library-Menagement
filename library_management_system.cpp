//kütüphaneler
#include <iostream>   //giriş çıkış işlemleri için(cin, cout)
#include <fstream>    //dosya işlemleri için
#include <string>   //string işlemleri içi
#include <cstdlib>   //exit() için
#include <Windows.h>  //konsol işlemleri(cls, pasuse) için
#include <cstdio>   //C dilinden mirastır, standart giriş çıkış işlemleri için
#include <vector>  //vector diziler(dinamik diziler) için
...
int main()
{
    ...
    system("cls"); //sayfayı yenileyelimki renkli sayfa gelsin:)

    string username = kullanici_girisi();
    while (true)
    {
        if (!username.empty())  //username boş değilse devam eder //empty() string kütüphanesindendir
        {
            if (username == "admin") 
            {
                adminmenu(username);
...
string kullanici_girisi()
{
    ...
    cout << R"( PROGRAMI SONLANDIRMAK ICIN USERNAME :[z] TUSLAYINIZ.
                                         ...
                                         #+#        #+#       #+#    #+#    #+#
                                        ########## ###       ###     ########
)";//R"(xxxx)" seklindeki dizin coutta \n ve tekrarli satrilari engeller ascii sanat?nda kullna?rl?r
    ...
    if (username == "z")
    {
        HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);// Konsolun arka plan ve yazi rengini ayarla
        ...
        cout << R"( 
                                         ...
                                         #+#        #+#       #+#    #+#    #+#
                                        ########## ###       ###     ########
                   )";//r"(xxxx)" seklindeki dizin coutta \n ve tekrarli satrilari engeller ascii sanatinda kullnairlir
        exit(0);  //çıkış
    }
    else
    {
...
string kimlik_dogrula(string username, string password)
{
    ...
    while (dosya >> satirUsername >> satirPassword)             //dosya içindek, kullanıcı adı ev şifreleri okur true dönerse devam eder
    {
...
void Kitapara()
{
    ...
    {
        if (satir.find(arananKelime) != string::npos)  //npos ile satırın sonuna geldiğimizi kontrol ediyoruz
        {
            cout << satir << endl;
        }
    }
...
void Kitapekle()
{
    ...
    getline(cin, yeniKitap.sayfa_sayisi);

    ofstream dosya(dosyaAdi, ios::app);  //app en sona ekleme yapıyor //app yerine out kullansaydık öncekileri silip sıfırdan ekleme yapıcaktı
    dosya << yeniKitap.isim << " / " << yeniKitap.yazar << " / " << yeniKitap.sayfa_sayisi << endl;
    dosya.close();
...
void Kitapsil()
{
    ...
    cin.ignore();
    getline(cin, silinecekKitap); //silinecek kitabın adını istiyor //getline kullanma nedeni içinde tanımlı bir \n olması
    ...
    while (getline(dosya, satir)) 
    {
        ...
        {
            kitaplar.push_back(satir);         // push_back = vector sonuna eleman -satir degiskeni- ekler 
        }
    }
    dosya.close();
    ...
    if (kitapBulundu) 
    {
        ofstream yeniDosya(dosyaAdi);
        for (const string &kitap : kitaplar) 
        {
            yeniDosya << kitap << endl;
        }
        yeniDosya.close();
...
void Kullaniciekle()
{
    ...
    getline(cin, parola);               //parola girmeyi istiyor //getline kullanma nedeni içinde tanımlı bir \n olması
...
void kullanicisil()
{
    ...
    cin.ignore();
    getline(cin, silinecek);    //silinecek kullanıcının adını istiyor
    ...
    while (getline(dosya, satir)) 
    {
        if (satir.find(silinecek) != string::npos) //npos satırın sonuna geldiğimizi söyler  
        {
            kitapBulundu = true;
        }
        else {
...
void kullanicilistele()
{
    ...
    while (getline(dosya, satir))   
    {
        if (satir.find(arananKelime) != string::npos)  //find() string kütüphanesinindir aradığımız kelimeyi bulmaya yarar
        {
            cout << satir << endl;
        }
    }
...
void kitapiadeet()
{
    ...
    cout << "iade edeceginiz kitabin ismi nedir: ";
    ...
    while (getline(dosya, satir)) 
    {
        if (satir.find(oduncKitap) != string::npos) //npos satırın sonuna geldiğimizi söyler
        {
            kitapBulundu = true;
        }
        else 
